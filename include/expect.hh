...
#define EXPECT_INIT()           Expect::init(__FILE__, __LINE__)

/** Print automated test results. */
#define EXPECT_FINI()           Expect::fini(__FILE__, __LINE__)

/** Verify value of expression is exactly the expected value. */
#define EXPECT_EQ(exp, obs)     Expect::eq(__FILE__, __LINE__, exp, obs, #obs)

/** Verify value of expression is not the same as the disallowed value. */
#define EXPECT_NE(exp, obs)     Expect::ne(__FILE__, __LINE__, exp, obs, #obs)

/** Verify value of pointer expression is a null */
#define EXPECT_NULL(obs)        Expect::null(__FILE__, __LINE__, obs, #obs)

/** Verify value of pointer expression is not a null */
#define EXPECT_NOT_NULL(obs)    Expect::not_null(__FILE__, __LINE__, obs, #obs)
...
namespace Expect {
    ...
    extern void          init(const char *file, int line);
     ...
     */
    extern int           fini(const char *file, int line);
    ...
    int eq(const char *file, int line, T const &exp, U const &obs,
           std::string const &src)
    {
        if (exp == obs) {
            report_pass(file, line);
            return 0;
        }
        report_fail(file, line)
     ...
     */
    template<typename T, typename U>
    int ne(const char *file, int line, T const &exp, U const &obs,
           std::string const &src)
    {
        if (exp != obs) {
            report_pass(file, line);
     ...
     */
    template<typename U>
    int null(const char *file, int line, U const &obs,
             std::string const &src)
    {
        if (!obs) {
            report_pass(file, line);
     ...
     */
    template<typename U>
    int not_null(const char *file, int line, U const &obs,
                 std::string const &src)
    {
        if (obs) {
            report_pass(file, line);
