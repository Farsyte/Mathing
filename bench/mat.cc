...
void              mat_init()
{
    Bench t(NN * 6 + N * 4, R, "Mat init with rand()");
    for (int rep = 0; rep < R; ++rep) {
        for (int i = 0; i < N; ++i) {
            cv(i) = rand() * 2.0 / RAND_MAX - 1;
            rv(i) = rand() * 2.0 / RAND_MAX - 1;
        }
        for (int r = 0; r < N; ++r) {
            for (int c = 0; c < N; ++c) {
                m1(r, c) = rand() * 2.0 / RAND_MAX - 1;
                m2(r, c) = rand() * 2.0 / RAND_MAX - 1;
                m3(r, c) = rand() * 2.0 / RAND_MAX - 1;
            }
        }
    ...
    for (int reps = 0; reps < 1000; reps++) {
        ...
        if (total_sec > finish_sec) {
            cerr << "\nBenchmark ending after " << reps << " repetitions of the suite;"
                 << "\nTotal elapsed time: " << str(total_sec, 0, 0) << " seconds" << endl;
            break;
        }
    }
}
