#include "vec.hh"
#include "bench.hh"
#include "exception.hh"
#include "backtrace.hh"
#include "timer.hh"

#include "expect.hh"

#include <cstdlib>

#include <iostream>
using std::cout;
using std::cerr;
using std::endl;

// when reducing N, be sure none of the individual
// benchmark sections run too quickly.

const int      N = 1000000;

const double   suite_sec = 5.0;
const double   finish_sec = 30.0;

int            R = 1;

Vec<N, double> v1;
Vec<N, double> v2;
Vec<N, double> v3;

void           vec_init()
{
    Bench t(N * 3, R, "Vec init with rand()");
    for (int rep = 0; rep < R; ++rep) {
        for (int i = 0; i < N; ++i) {
            v1(i) = rand() * 2.0 / RAND_MAX - 1;
            v2(i) = rand() * 2.0 / RAND_MAX - 1;
            v3(i) = rand() * 2.0 / RAND_MAX - 1;
        }
        EXPECT_NE(normsq(v1), 0);
        EXPECT_NE(normsq(v2), 0);
        EXPECT_NE(normsq(v3), 0);
    }
}

void vec_assign()
{
    Bench t(N, R, "Vec = Vec");
    for (int i = 0; i < R; ++i) {
        v1 = v2;
    }
}

void vec_addassign()
{
    Bench t(N, R, "Vec += Vec");
    for (int i = 0; i < R; ++i) {
        v1 += v2;
    }
}

void vec_subassign()
{
    Bench t(N, R, "Vec -= Vec");
    for (int i = 0; i < R; ++i) {
        v1 -= v2;
    }
}

void vec_add()
{
    Bench t(N, R, "Vec = Vec + Vec");
    for (int i = 0; i < R; ++i) {
        v1 = v2 + v3;
    }
}

void vec_sub()
{
    Bench t(N, R, "Vec = Vec - Vec");
    for (int i = 0; i < R; ++i) {
        v3 = v1 - v2;
    }
}

void vec_mul()
{
    Bench t(N, R, "Vec *= Num");
    for (int i = 0; i < R; ++i) {
        v1 *= 1.01;
    }
}

void vec_div()
{
    Bench t(N, R, "Vec /= Num");
    for (int i = 0; i < R; ++i) {
        v1 /= 1.01;
    }
}

void vec_dot()
{
    double s = 0;
    Bench  t(N, R, "Num += Vec * Vec");
    for (int i = 0; i < R; ++i) {
        s += v1 * v2;
    }
    EXPECT_NE(s, 0);
}

void bench_vecmul_body()
{
    vec_init();
    vec_assign();
    vec_addassign();
    vec_subassign();
    vec_add();
    vec_sub();
    vec_mul();
    vec_div();
    vec_dot();
}

void bench_vecmul()
{

    cout << endl
         << "Benchmarking with N=" << N << "\n"
         << "All durations reported in ns per element.\n"
         << "Each vecrix uses " << sizeof(v1) << " bytes of storage.\n"
         << "Target time per pass: " << suite_sec << " seconds.\n"
         << "Target time for run: " << finish_sec << " seconds.\n"
         << endl;

    // The first run is subject to Cold Cache effects.
    // Ignore its runtime.
    R = 1;
    bench_vecmul_body();

    for (;;) {
        Timer reptime;
        bench_vecmul_body();
        double sec = reptime.elapsed_sec();

        // Tune R to get elapsed time to suite_sec
        if (sec < suite_sec / 10) {
            // Insufficient elapsed time to allow us to
            // compute the R that gets us close to suite_sec.
            R *= 10;
        } else {
            // We are within an order of magnitude of the
            // right elapsed time. Pick an R and go with it.
            cerr << "For R=" << R << ", T=" << str(sec, 0, 2);
            R = (R * suite_sec / sec) + 0.5;
            cerr << "; want T=" << suite_sec;
            cerr << ", will use R=" << R << endl;
            Bench::enable();
            break;
        }
    }

    Timer benchtime;
    for (int reps = 0; reps < 1000; reps++) {
        bench_vecmul_body();
        double total_sec = benchtime.elapsed_sec();
        if (total_sec > finish_sec) {
            cerr << "\nBenchmark ending after " << reps << " repetitions of the suite;"
                 << "\nTotal elapsed time: " << str(total_sec, 0, 0) << " seconds" << endl;
            break;
        }
    }
}

int main()
{
    try {
        bench_vecmul();
        Bench::dump_log(cout);
    } catch (Exception &e) {
        cerr << "benchmark terminated by Exception: " << e << endl;
        print_backtrace();
        abort();
    }
}
